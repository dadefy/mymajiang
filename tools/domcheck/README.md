...
| `check-match-points.mjs` | 不连服务端 | 本地 dist | — | 头像记整局累计（跨小场不清零）、一小场只弹本小场四家分数、打满 8 小场后按掉最后一屏才出整局结算记录（顶部时间行 + 四行玩家明细） |
...
| `check-settlement.mjs` | `PAGE`（默认本机 3000） | 本地 dist | ✓ 4 把 | 结算界面显示番型与「谁给的牌」；`FULL_MATCH=1` 再打到整局结算，验开始时间 / 耗时与四行玩家明细（要几分钟） |
