...
const finalResult = {
  ...
  balances: USER_IDS.map((playerId, seat) => ({ playerId, balance: [1022, 992, 992, 994][seat] })),
};
push({
  match: {
    roomId: 'r',
...
console.log('PASS: 头像记整局累计（跨小场不清零）；一小场弹本小场四家分数（按座位、带本场累计、不含账号）；打满 8 小场后按掉最后一屏才出整局结算记录（含账号入账与余额）');
dom.window.close();
process.exit(0);
