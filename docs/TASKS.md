...
- **完成（图片）**：服务端链路与 COS 云驱动见 `PROJECT_STATUS.md` 4.18；
  ...
  - 新增 6 项测试，全量 33 文件 / 279 项通过
- **未做**：**语音（录音）** —— 上传与协议两侧都已支持 `voice` / `voiceSeconds`，
  缺的是录音与编码（`getUserMedia` + `MediaRecorder`）。
- **已知限制**：选图依赖真实 DOM，原生 APK 若没有 DOM 模拟会退化成「发不了图」，需补原生相册桥接。
...
- 测试：33 个文件 / 279 项全通过（A3 图片上传合并后本机实测）
- 已完成：规则引擎、领域逻辑、服务端（HTTP + WebSocket + PostgreSQL）、
  ...
  **图片上传（A3 客户端侧）**、HTTP/WebSocket 单端口联网调试与浏览器调试客户端
- **还没做的开发只有语音（录音）；其余是人工项：D1/D2 与图片上传待四端真机验收，
  以及部署、备案、隐私政策与协议文本。**
