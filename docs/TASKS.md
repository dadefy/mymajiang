...
| C5 | 棋牌类上架的合规门槛（游戏版号 / 商店准入） | 麻将属于棋牌类，国内商店普遍要求游戏版号、准入极严。**先确认这条路走得通，再投入打包与签名**；内测（浏览器打开链接）不受影响 |

---

## 第二波：等第一波完成

### A3 · 对象存储与图片语音上传 〔W · 已完成〕

- **依赖**：C1（存储账号）
- **范围**：新增 `apps/server/src/object-storage.ts` + 上传接口；客户端补图片/语音发送
- **验收**：能上传图片并作为群消息发出；无存储配置时接口返回 501 而不是崩溃
- **完成（图片）**：服务端链路与 COS 云驱动见 `PROJECT_STATUS.md` 4.18；
  客户端侧见 4.25。要点：
  - 新增第三个环境缝 `UploadTransport`（把二进制 PUT 到签名地址，**不带我们的令牌**）
  - `ClientFlow.uploadGroupImage(群 ID, { bytes, contentType })` 串起「签发 → 直传 → 用对象键发消息」，
    且不依赖当前页面（浏览器调试面板在主页里就能发图）
  - LayaAir 侧 `LayaUploadTransport` + `ui/file-picker.ts` + 群聊页「图片」按钮；
    浏览器侧 `FetchUploadTransport` + 调试面板发图入口
  - 直传没成功就不发消息；没配存储、类型/大小不合规、直传失败各给一句人话
- **完成（语音）**：见 `PROJECT_STATUS.md` 4.27。录音器放在 `apps/client/src/browser/`
  （`MediaRecorder` 是标准 Web API，两个环境共用）；`sendVoice` 复用同一条三步链路；
  界面上点一下开始、再点一下停止并发送，到 60 秒自动停。
  两个坑写在代码里：`mimeType` 必须截掉 `;codecs=…`（服务端按精确值比对白名单）、
  时长要传 1–60 的整数秒。
- **已完成（浏览器调试客户端）**：消息列表里图片显示缩略图（点开看原图）、语音显示播放控件，
  见 `PROJECT_STATUS.md` 4.32。要点：服务端早就把 `content` 换成了**带时效的签名读取地址**，
  所以这是纯渲染层的事 —— 但那个地址几十秒就过期，必须在客户端抓成本地 blob 地址缓存。
- **未做（LayaAir 渲染层）**：`chat-model.ts` 里图片与语音仍是 `[图片]` / `[语音 N 秒]` 占位。
  那一端要 `Laya.Image` + 网络加载 / `Laya.SoundManager`，**需要 LayaAir IDE 构建后才能验证**，
  所以与真机 UI 一起做。
- **已知限制**：录音与选图都依赖真实 Web API（`MediaRecorder` / `<input type="file">`），
  **原生 APK 下需要另做桥接**，否则退化成「录不了音 / 发不了图」；
  浏览器只在 HTTPS 或 localhost 下给麦克风权限。
...
- **完成**：房间页已接入真实牌面资源与服务端 `actions` 权限列表，支持选择同花色三张换牌、
  定缺、强制先打缺门牌、出牌、碰、直杠、暗杠、补杠、点炮胡、自摸胡、过牌及单局/整场结算。
  新增纯逻辑测试覆盖牌面映射、换牌选择和定缺出牌限制；LayaAir Web 构建通过。
  后续一轮界面精修（2026-09-16）：出牌改为**两次点击确认**（同一张实体牌点两次才出，
  同牌面不同实体牌互不确认，换回合自动清选）；四家头像按方位显示并带本局净输赢与庄/定缺徽标；
  四方牌桌布局（外圈手牌、内圈碰杠、中央弃牌区、方向盘 + 行动倒计时 + 余牌）；
  换三张按实体牌计数。已提交换牌后保留一行等待状态，避免按钮消失后看起来像卡住。
  **剩余验收**：四台真机连接真实服务端连续打完 8 局。
...
- **完成**：完整记录见 `PROJECT_STATUS.md` 4.24。要点：
  - `Screen` 新增 `chat`；**实时帧改为按当前页面分发** —— 此前 `ClientFlow` 是
    「不在房间页就丢掉」，所以离开房间页后群消息推送全部丢失，这是本任务真正要修的。
  - 进群建立**只订阅群聊**的连接（`auth` 省略 `roomId`）并订阅该群；回主页退订并断开。
  - 发消息按 `messageId` 去重：REST 返回值与实时推送两条路都会到，只留一条。
  - 历史翻页接上领域层的游标分页；渲染层新增 `chat-model.ts`（纯函数）+ `ChatPage.ts`，
    主页群列表行改为可点击。
  - **顺带修掉三处接口缺口与一处协议漂移**：消息路由绕过游标分页（文档写了、HTTP 层没有）、
    消息视图缺发送者昵称、群详情缺调用者角色、客户端消息类型写成 `sticker`/`room-invite`。
  - 新增测试：`apps/apk/test/chat-model.test.ts`（8 项）、`apps/client/test/flow.test.ts` 群聊 7 项、
    `apps/server/src/app.test.ts` 分页与角色 2 项。全量 33 文件 / 273 项通过。
- **剩余验收**：两个客户端连真实服务端实时互通；与 A1/D1 一样需要真机。
