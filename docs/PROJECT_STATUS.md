...
> 更新时间：2026-09-17  
> 当前阶段：服务端功能完备；**网页牌桌界面精修已完成**（四方布局、两次点击出牌、四家头像与整局累计、行动倒计时），**一整局的积分口径已按「8 小场一局、整局结算才入账」落地**（小场结束只在牌桌上弹 3 秒数字，不亮牌不写牌型；整局结算记录带开始时间 / 耗时与逐家四行明细）；APK / LayaAir 侧仍待四端真机验收
