...
- 当前共有 34 个测试文件、300 项自动化测试（幂等键与弱网重试合并后本机全量实测；
  含浏览器传输层、D1 牌桌选择逻辑、D2 群聊展示模型与群聊页面流、A3 上传链路、
  幂等键与重试、B3 群消息分页与「按库取回撤回」用例）。
...
- 最近一次结果：300 项全部通过（幂等键与弱网重试合并后本机实测）。
...
2. 图片上传已完成：服务端链路（见 4.18）、COS 云驱动与**客户端选图上传**（见 4.25）都已就位；
   剩下的是**语音（录音）**与真机验收。
...
- **语音消息还没有**：录音与编码那一段（`getUserMedia` + `MediaRecorder`）尚未实现，
  上传与协议两侧都已经留好位置。
...
1. **语音消息（录音）**：图片这条链路已经打通（见 4.25），语音缺的是录音与编码那一段。
2. **部署、安全与合规**：完成正式联网前准备。
3. **真机验收**：四端连续打完 8 局；两个客户端实时互通群消息与图片。
