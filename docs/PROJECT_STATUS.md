...
2. 图片和语音上传的服务端链路已完成（见 4.18）：签发直传、类型与大小校验、归属校验、
   带签名的读取地址，本地驱动可完整跑通。**还差云驱动**（腾讯云 COS）与客户端上传界面，
   云驱动需要存储账号。
...
- 对象存储目前只有**本地磁盘驱动**：图片与语音能完整跑通与测试，但生产环境需要云驱动
  （腾讯云 COS），否则进程重启后文件仍在原地、多实例之间也不共享。
  云驱动还差：签名算法的实现（COS 用 HMAC-SHA1 的规范化请求串）或引入官方 SDK。
