...
胡牌亮牌规则（2026-09-16 确认）：前两家胡牌后仅标记已胡，不向其他玩家公开手牌；第三家胡牌结束本小场（牌墙耗尽仍按流局结算）。一小场结束统一亮出四家手牌与副露，逐家列出本小场净输赢，零分也显示。胡牌牌面保存在服务端快照中，支持重启恢复。

一整局的积分口径（2026-09-17 确认）：**一局游戏 = 8 小场**。① 头像下面记的是整局累计（跨小场不清零）；② 每小场结束时弹一次那一小场四家的分数变化（数字放大、赢红输绿），随后开下一小场；③ 打满 8 小场（或中途解散）之后才出**整局结算记录**，逐家给出本小场、本场累计、账号入账与入账后的余额；④ **账号积分只在出结算记录的那一刻改**，打的过程中一律不动。术语按这套统一：一小场叫「小场」，8 小场合起来叫「一局」。
...
> 更新时间：2026-09-17  
> 当前阶段：服务端功能完备；**网页牌桌界面精修已完成**（四方布局、两次点击出牌、四家头像与整局累计、行动倒计时），**一整局的积分口径已按「8 小场一局、整局结算才入账」落地**，整局结算记录已带上开始时间 / 耗时与逐家四行明细（头像 + 昵称 + id 号 + 本局积分变化）；APK / LayaAir 侧仍待四端真机验收
