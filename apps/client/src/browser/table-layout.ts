...
export function installTableLayout(): void {
  ...
  style.textContent = `
    ...
    #board .seat.left .hand{left:8%}#board .seat.right .hand{right:8%}
    ...
    #board .seat.left .tiles-area>.melds{left:17%}#board .seat.right .tiles-area>.melds{right:17%}
    ...
    #board .seat.left .meld-group .chip,#board .seat.right .meld-group .chip{width:clamp(14px,1.35vw,19px);height:27px;font-size:10px;white-space:nowrap}
    ...
    #board .seat-card>.ops{justify-content:center;z-index:3;max-height:9%;overflow:auto}
    #board .seat.bottom .ops{left:20%;bottom:1.5%;width:60%}#board .seat.top .ops{left:26%;top:13%;width:48%}
    #board .seat.left .ops{left:1%;top:73%;width:23%}#board .seat.right .ops{right:1%;top:73%;width:23%}
